...
INSTALL_PREFIX="/data/data/com.termux/files/usr/llvm"
ANDROID_TRIPLE="aarch64-unknown-linux-android35"

# 使用新安装的 LLVM 23 作为构建工具
...
MOTHER_RT="$INSTALL_PREFIX/lib/clang/23/lib/linux/libclang_rt.builtins-aarch64-android.a"

# ================= 编译参数 =================
# 64KB 页面对齐 + 硬编码运行时库路径 + 注入静态 builtins
LDFLAGS_FIX="-Wl,-z,max-page-size=65536 -Wl,-z,common-page-size=65536 -Wl,-z,separate-loadable-segments -L$TERMUX_PREFIX/lib -L$NEW_LIB_DIR -Wl,-rpath,$TERMUX_PREFIX/lib $MOTHER_RT"
...
cmake -G Ninja \
    ...
    -DLLVM_ENABLE_PROJECTS="clang;lld;mlir;polly;bolt;clang-tools-extra" \
